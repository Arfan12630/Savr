...
const ViewRestaurantLayout: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://127.0.0.1:8000/get-layout?name=${restaurantInfo.name}&address=${restaurantInfo.address}`
      )
      .then(response => {
  ...
  useEffect(() => {
    if (layout && reservationData && restaurantInfo) {
      axios
        .get(
          `http://127.0.0.1:8000/auto-assign-tables?name=${restaurantInfo.name}&address=${restaurantInfo.address}&party_size=${reservationData.party_size}&time=${reservationData.time}&occasion=${reservationData.occasion}`
        )
        .then(response => {
          console.log(response.data);
