...
const MenuDisplay: React.FC = () => {
  ...
  const isOwner = true;
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/get-all-menu-html').then(res => {
  ...
  const handleAddToOrder = () => {
    ...
    const itemToAdd: MenuItem = {
      ...
      selectedEnhancements:
        selectedEnhancements.length > 0 ? selectedEnhancements : undefined,
    };
  ...
  const handleMenuItemClick = (event: React.MouseEvent) => {
    ...
    if (menuItem) {
      ...
      const allParagraphs = Array.from(
        menuItem.querySelectorAll('p:not(.price)')
      );
      ...
      const isCombo =
        displayName.toLowerCase().includes('combo') ||
      ...
      if (isCombo && name) {
        ...
        if (
          (!name.toLowerCase().includes('combo') &&
            section.toLowerCase().includes('combo')) ||
          (section.toLowerCase().includes('family') &&
            name.toLowerCase().includes('kids'))
        ) {
  ...
  const handlePrevious = () => {
    ...
    if (currentItemIndex > 0) {
      ...
      const prevGroupIndex =
        currentGroupIndex > 0 ? currentGroupIndex - 1 : menuHtml.length - 1;
  ...
  const handleNext = () => {
    ...
    if (
      menuHtml[currentGroupIndex] &&
      currentItemIndex < menuHtml[currentGroupIndex].length - 1
    ) {
      setCurrentItemIndex(currentItemIndex + 1);
    } else {
      // Move to the next group
      const nextGroupIndex =
        currentGroupIndex < menuHtml.length - 1 ? currentGroupIndex + 1 : 0;
  ...
  const extractEnhancements = (): string[] => {
    ...
    const addonsSection =
      menuContainerRef.current.querySelector('.menu-addons');
      ...
      {selectedItem && (
        <div className="selected-item-modal">
          <div className="selected-item-content">
            ...
            {selectedItem.section && !selectedItem.isCombo && (
              <p className="selected-item-section">{selectedItem.section}</p>
            )}
            {selectedItem.price && (
            ...
            {selectedItem.selectedOption && (
              <div className="selected-option">
                <p>
                  Selected: <strong>{selectedItem.selectedOption}</strong>
                </p>
              </div>
            )}
              ...
              {currentItem && (
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: cleanHTML(currentItem.html),
                  }}
                />
              )}
            </div>
