...
const DroppableArea = () => {
    ...
    setTables(prev =>
      prev.map(table => {
        if (table.id === active.id) {
          ...
          const overlapWithChairs = chairs.some(c =>
            isOverlapping(
              newX,
              newY,
              TABLE_WIDTH,
  ...
  const updateTableSize = (id: string, width: number, height: number) => {
    setTables(prev =>
      prev.map(table => (table.id === id ? { ...table, width, height } : table))
    );
  };
  ...
  const updateTableNumber = (id: string, tableNumber: string) => {
    setTables(prev =>
      prev.map(table => (table.id === id ? { ...table, tableNumber } : table))
    );
  };
  ...
  const saveLayout = () => {
    ...
    axios({
      method: 'post',
      url: 'http://127.0.0.1:8000/save-layout',
      data: {
        layout,
        restaurantCardData,
      },
